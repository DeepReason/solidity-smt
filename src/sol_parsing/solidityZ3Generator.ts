import { SolidityData, StateVarData } from './analyze_solidity';
import { BitVec, BitVecSort, Expr, SMTArray, SMTArraySort, Z3Obj } from '../z3/z3';
import { ElementaryVarType, MappingVarType, VarType, VarTypeKind } from './vartype';
...
function getGlobalVarZ3Generator(stateVarData: StateVarData[]): Map<string, AnnotatedZ3Generator> {
  const globalZ3Vars = new Map<string, AnnotatedZ3Generator>();
  for (const stateVar of stateVarData) {
    try {
      const unslottedGenerator = solidityVarToUnslottedZ3Generator(stateVar.type, stateVar.contractName);
      assert(stateVar.slot !== undefined);
      if (stateVar.slot[0] !== stateVar.slot[1]) {
        throw new Error('Unsupported: Global vars must be in a single slot');
      }
      const generator = (z3: Z3Obj, global_storage: GlobalStorageZ3) => {
        const slot = z3.BitVec.val(stateVar.slot![0], 256);
        return unslottedGenerator(z3, global_storage)(slot, stateVar.intraSlot?.[0]);
      };
      globalZ3Vars.set(stateVar.name, generator);
    } catch (e: any) {
      console.log('WARNING: ' + e.message);
      console.log('WARNING: Skipping global var: ' + stateVar.name);
    }
  }
  return globalZ3Vars;
}

export function solidityDataToZ3Generators(solidityData: SolidityData): Z3SolidityGenerators {
  const globalVarZ3Generators = getGlobalVarZ3Generator(solidityData.stateVarData);
  return {
    solidityData,
