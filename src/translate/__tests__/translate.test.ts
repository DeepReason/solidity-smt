...
describe('Test translate', () => {
  ...
  describe('Test WETH9 2', () => {
    const parsedSolidity = JSON.parse(
      fs.readFileSync(path.join(__dirname, 'examples/parsed_solidity_WETH9_2.json'), 'utf8'),
    );
    const exposedImmutables = JSON.parse(
      fs.readFileSync(path.join(__dirname, 'examples/exposed_immutables_WETH9_2.json'), 'utf8'),
    );
    ...
    it('Translate expression', async () => {
      const result = await unerroredTranslation('msg.sender', 'WETH9');
      expect(result.warnings).toEqual([
        'skipping variable name (Type string not supported)',
        'skipping variable symbol (Type string not supported)',
      ]);
    });
    ...
    it('Translate equality', async () => {
      const result = await unerroredTranslation('decimals == 18', 'WETH9');
      expect(result.warnings).toEqual([
        'skipping variable name (Type string not supported)',
        'skipping variable symbol (Type string not supported)',
      ]);
