...
DUMPING EXPRESSIONS
*/
function exprToSolverClaim(expr) {
    const z3 = expr.ctx;
    return expr.eq(z3.Const('__deepreason_claim_tmp', expr.sort));
}
export function dumps_expr(expr) {
    let z3;
...
LOADING EXPRESSIONS
*/
export function loads_expr(z3, expr_str) {
    const solver = new z3.Solver();
...
export function loads_func_decl(z3, dec_str) {
    const solver = new z3.Solver();
...
export async function loads_model(z3, model_str) {
    ...
    for (const dv_str of model_str.trim().split(';')) {
        ...
        if (val[0] != '&') {
            m.updateValue(loads_func_decl(z3, f_str), loads_expr(z3, val));
        }
        else {
            const entries = val.slice(1).split('&');
            const else_val = loads_expr(z3, entries.pop());
            const f = loads_func_decl(z3, f_str);
            const interp = m.addFuncInterp(f, else_val);
            for (const entry of entries) {
                const args = entry.split('->').map((x) => loads_expr(z3, x));
                const v = args.pop();
